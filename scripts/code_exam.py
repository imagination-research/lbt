...
if __name__ == "__main__":
    ...
    if teaching_plans != "no demo":
        ...
        # the columns are: question, answer, test
        if "answer" not in teaching_dataset.features:
            LOGGER.info(
                f"Cannot find `answer` field in {args.teaching_dataset_file}"
            )
            raise NotImplementedError
        else:
            LOGGER.info(
                f"Use the `answer` field in {args.teaching_dataset_file} as the"
                " demonstration."
            )
    ...
    if "answer" not in exam_dataset.features and "canonical_solution" in exam_dataset.features:
        exam_dataset = exam_dataset.rename_columns({"canonical_solution": "answer"})

    if "question" not in exam_dataset.features and "prompt" in exam_dataset.features:
        exam_dataset = exam_dataset.rename_columns({"prompt": "question"})
    ...
    if "rationale" not in exam_dataset.features:
        ...
        LOGGER.info(
            f"We do not use `rationale` to measure"
            " scores."
        )
    else:
        LOGGER.info(
    ...
    for teaching_plan in tqdm(teaching_plans):
        ...
        output_item["task_id"] = s_exam_dataset["task_id"] # New in code datasets
